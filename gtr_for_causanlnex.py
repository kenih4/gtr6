...
#fname = str(df_set.loc['title']).replace("1","").strip() + ".dat";
#fname = str(df_set.loc['title']) + ".dat";
fname = str(df_set.loc['title']).replace("1","").strip().splitlines()[0]  + ".xlsx";
print(fname)
...
for n, s in enumerate(sig, 0):
	header += df_sig.loc[n]['label'] + "	"
#	print(df_sig.loc[n]['label'])
print(header)
f.write(header + "\n")
"""
...
#		df = pd.DataFrame({ 'key':sig[0].d.keys()}	)
		...
		ltmp =""
		skip=0
		for key in sig[0].d.keys():
#			print(str(key) + "	" + str(sig[0].d[key]))
			if skip == 0:
				for n, s in enumerate(sig, 0):
					if df_sig.loc[n]['label'] != "kick":					
						if df_sig.loc[n]['label'] == "uJ":
							ltmp += str(sig[n].d[key]*10000000000) + "	"
							list_line.append(sig[n].d[key]*10000000000)
						else:
							ltmp += str(sig[n].d[key]) + "	"
							list_line.append(sig[n].d[key])
#				f.write(ltmp + "\n")
				df.loc[key] = list_line
				list_line.clear()
				skip = 5
			skip = skip -1
			ltmp =""
#			f.write(str(key) + "	" + str(sig[12].d[key]) + "	" + str(sig[11].d[key]) + "	" + str(sig[7].d[key]) + "	" + str(sig[6].d[key]) + "	" + str(sig[4].d[key]) + "	" + str(sig[2].d[key]) + "	" + str(sig[8].d[key]) + "\n")
		print("A-----------------------------------------------------------------------")
#		print(df)
		print("len(df):	" + str(len(df)))
		print("B-----------------------------------------------------------------------")
		if not sig[0].d.keys():
			print('NO sig[0].d.keys() !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
		else:
			startkey=(next(iter(sig[0].d)))
			print("startkey:	" + str(startkey))	
#		print("head().key:	" + str(df.head(1).key))	DAME					
#		df=df.drop(startkey)
#		df=df.drop(range(0,startkey), errors='ignore')	DAME
#		df=df.iloc[range(startkey), :]	DAME
#		df=df.drop(range(0,df.index.get_loc(startkey)), errors='ignore')	DAME
...
#		print(df)		
		print("len(df):	" + str(len(df)))		
#		df.to_excel(fname, sheet_name='sig')
		df.to_excel(fname, sheet_name='sig', index=False)		
		print("C-----------------------------------------------------------------------")
...
#	'fargs': keys,  # 関数の引数 (フレーム番号を除く)
	'interval': 10 * 1000,  # グラフを更新する間隔 (ミリ秒)
}
